...
//退休职工工资汇总表
class SalLtxSumReport extends PureComponent {
    state = {
        src: '',
        clientHeight: '0px',
        displayReport: 'none',
    ...
    constructor(props) {
        ...
        //日期
        let yearmonth = []
        yearmonth.push(moment(sessionStorage.getItem('salLtxYearMonth'), monthFormat))
        yearmonth.push(moment(sessionStorage.getItem('salLtxYearMonth'), monthFormat))
    ...
    handleOperator = (type) => {
        ...
        let src = ''
        if (QUERY === type) {
            src = baseURL + '/ureport/preview?_t=0&_u=file:salLtxSum.ureport.xml' + condition
        } else {
            src = baseURL + '/ureport/excel?_u=file:salLtxSum.ureport.xml' + '&_n=' + sessionStorage.getItem('salLtxYearMonth') + '退休职工工资汇总表' + condition
        }
        this.setState({
            src,
...
export default SalLtxSumReport
