...
//内聘职工工资汇总表
class SalNpSumReport extends PureComponent {
    state = {
        src: '',
        clientHeight: '0px',
        displayReport: 'none',
    ...
    handleOperator = (type) => {
        ...
        let src = ''
        if (QUERY === type) {
            src = baseURL + '/ureport/preview?_t=0&_u=file:salNpSum.ureport.xml' + condition
        } else {
            src = baseURL + '/ureport/excel?_u=file:salNpSum.ureport.xml' + '&_n=' + sessionStorage.getItem('salNpYearMonth') + '内聘职工工资汇总表' + condition
        }
        this.setState({
            src,
