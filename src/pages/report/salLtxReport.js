...
//退休职工工资清单表
class SalLtxReport extends PureComponent {
    state = {
        src: '',
        clientHeight: '0px',
        displayReport: 'none',
    ...
    handleOperator = (type) => {
        ...
        let src = ''
        if (QUERY === type) {
            src = baseURL + '/ureport/preview?_t=0&_u=file:salLtx.ureport.xml' + condition
        } else {
            src = baseURL + '/ureport/excel?_u=file:salLtxSum.ureport.xml' + '&_n=' + sessionStorage.getItem('salLtxYearMonth') + '退休职工工资清单表' + condition
        }
        this.setState({
            src,
