...
//离休职工工资汇总表
class SalLxSumReport extends PureComponent {
    state = {
        src: '',
        clientHeight: '0px',
        displayReport: 'none',
    ...
    handleOperator = (type) => {
        ...
        let src = ''
        if (QUERY === type) {
            src = baseURL + '/ureport/preview?_t=0&_u=file:salLxSum.ureport.xml' + condition
        } else {
            src = baseURL + '/ureport/excel?_u=file:salLxSum.ureport.xml' + '&_n=' + sessionStorage.getItem('salLxYearMonth') + '离休职工工资汇总表' + condition
        }
        this.setState({
            src,
