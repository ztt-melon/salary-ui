...
import SalLtxQuery from './salLtxQuery'
import salLtxColumns from './column'
import {ajax, urls, Functions, PermissionButton,Constants} from '../../util'
import axios from "axios"

const Option = Select.Option

class SalLtx extends PureComponent {
    state = {
        pageNum: 1,
        loading: true,
        //
    ...
    displaySearch = () => {
        let index = PermissionButton.get(urls.salLtx.route).indexOf('查询')
        if (index >= 0) {
            return (
                <SalLtxQuery
    ...
    render() {
        ...
        return (
            <Fragment>
                <MyBreadcrumb msg={'工资管理/退休工资'}/>
                {this.displaySearch()}
                <div>
                    {/*<Button type="primary" icon="file" onClick={this.showModal}>批量修改</Button>*/}
                    <Modal
                        title="批量修改"
                        width={400}
